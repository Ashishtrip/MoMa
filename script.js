        ...
        class CSVExporter {
            static export(transactions) {
                ...
                a.download = 'moma-transactions.csv';
                a.click();
            }
        }
        ...
        class ChartManager {
            ...
            capitalize(str) { return str ? str.charAt(0).toUpperCase() + str.slice(1).replace('_', ' ') : ''; }
            getColor(value, minV, maxV) {
                if (minV === maxV) return '#808080';
                const norm = (value - minV) / (maxV - minV);
                const grey = Math.round(224 - norm * 198); // #e0e0e0 to #1a1a1a
